 ...
 */
RC_t BUTTON_Init() // The respective ISR attached to be called is handled by the OS
{
    return RC_SUCCESS;
}
...
boolean_t BUTTON_IsPressed(BUTTON_id_t button)
{
    ...
    switch (button)
    {
        case BUTTON_LEFT : if (ButtonLeft_Read() == 1) result = TRUE; break;
        case BUTTON_RIGHT : if (ButtonRight_Read() == 1) result = TRUE; break;
    }
