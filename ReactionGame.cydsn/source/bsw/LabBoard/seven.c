...
RC_t SEVEN_writeRandom() {
    ...
    // Set both displays to the same random value
    SEVEN_SELECT_Write(0);  // Select right display
    SEVEN_reg_Write(data);

    SEVEN_SELECT_Write(1);  // Select left display
    SEVEN_reg_Write(data);
...
{
    //Set software latch to cleared
    SEVEN__latch_value[0] = -1;
    SEVEN__latch_value[1] = -1;
    
    //Set the selection bit for the right display
    SEVEN_SELECT_Write(0);
    //Clear every bit
    SEVEN_reg_Write(0);

    //Set the selection bit for the left display
