...
/*******************************************************************************
* Function Name: PWM_Clock_RGB_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void PWM_Clock_RGB_Start(void) 
{
    /* Set the bit to enable the clock. */
    PWM_Clock_RGB_CLKEN |= PWM_Clock_RGB_CLKEN_MASK;
	PWM_Clock_RGB_CLKSTBY |= PWM_Clock_RGB_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: PWM_Clock_RGB_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void PWM_Clock_RGB_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    PWM_Clock_RGB_CLKEN &= (uint8)(~PWM_Clock_RGB_CLKEN_MASK);
	PWM_Clock_RGB_CLKSTBY &= (uint8)(~PWM_Clock_RGB_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: PWM_Clock_RGB_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void PWM_Clock_RGB_StopBlock(void) 
{
    if ((PWM_Clock_RGB_CLKEN & PWM_Clock_RGB_CLKEN_MASK) != 0u)
    {
...
#if defined(PWM_Clock_RGB__CFG3)
        CLK_DIST_AMASK = PWM_Clock_RGB_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = PWM_Clock_RGB_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* PWM_Clock_RGB__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(PWM_Clock_RGB_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(PWM_Clock_RGB_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: PWM_Clock_RGB_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void PWM_Clock_RGB_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        PWM_Clock_RGB_CLKSTBY &= (uint8)(~PWM_Clock_RGB_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: PWM_Clock_RGB_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void PWM_Clock_RGB_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = PWM_Clock_RGB_GetSourceRegister();
    uint16 oldDivider = PWM_Clock_RGB_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = PWM_Clock_RGB_CLKEN & PWM_Clock_RGB_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(PWM_Clock_RGB_DIV_PTR, clkDivider);
                PWM_Clock_RGB_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                PWM_Clock_RGB_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(PWM_Clock_RGB_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((PWM_Clock_RGB_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: PWM_Clock_RGB_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 PWM_Clock_RGB_GetDividerRegister(void) 
{
    return CY_GET_REG16(PWM_Clock_RGB_DIV_PTR);
}


/*******************************************************************************
* Function Name: PWM_Clock_RGB_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void PWM_Clock_RGB_SetModeRegister(uint8 modeBitMask) 
{
    PWM_Clock_RGB_MOD_SRC |= modeBitMask & (uint8)PWM_Clock_RGB_MODE_MASK;
}


/*******************************************************************************
* Function Name: PWM_Clock_RGB_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void PWM_Clock_RGB_ClearModeRegister(uint8 modeBitMask) 
{
    PWM_Clock_RGB_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(PWM_Clock_RGB_MODE_MASK));
}


/*******************************************************************************
* Function Name: PWM_Clock_RGB_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 PWM_Clock_RGB_GetModeRegister(void) 
{
    return PWM_Clock_RGB_MOD_SRC & (uint8)(PWM_Clock_RGB_MODE_MASK);
}


/*******************************************************************************
* Function Name: PWM_Clock_RGB_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void PWM_Clock_RGB_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = PWM_Clock_RGB_GetDividerRegister();
    uint8 oldSrc = PWM_Clock_RGB_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        PWM_Clock_RGB_MOD_SRC |= CYCLK_SSS;
        PWM_Clock_RGB_MOD_SRC =
            (PWM_Clock_RGB_MOD_SRC & (uint8)(~PWM_Clock_RGB_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: PWM_Clock_RGB_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 PWM_Clock_RGB_GetSourceRegister(void) 
{
    return PWM_Clock_RGB_MOD_SRC & PWM_Clock_RGB_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: PWM_Clock_RGB_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void PWM_Clock_RGB_SetPhaseRegister(uint8 clkPhase) 
{
    PWM_Clock_RGB_PHASE = clkPhase & PWM_Clock_RGB_PHASE_MASK;
}


/*******************************************************************************
* Function Name: PWM_Clock_RGB_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 PWM_Clock_RGB_GetPhaseRegister(void) 
{
    return PWM_Clock_RGB_PHASE & PWM_Clock_RGB_PHASE_MASK;
}
